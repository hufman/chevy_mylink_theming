...
tree = ET.parse('ani_right.svg')
root = tree.getroot()

layers = list(root)
new_layers = [l for l in layers if re.match('^New.*[0-9]', l.attrib['id'])]
...
for frame in new_layers:
	...
	particle_info = {
		...
		'y': random.uniform(128-3, 128+3),
		'speed_x': random.uniform(5, 6),
